...
<template>
  <div class="bg scoll">
    <div style="display: flex;width:calc(100% - 5px);padding-right: 5px;border-bottom: 1px solid var(--borderColor);-webkit-app-region: drag;">
        <label style="width:49px">文件：</label>
    ...
    <div class="scoll" style="display: grid;grid-template-columns: repeat(auto-fill, minmax(82px, 1fr));align-items: start;border-bottom: 1px solid var(--borderColor);">
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('浏览')}" @click="store.toggleView('浏览')"><i class="fa fa-book"></i> 浏览</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('编辑')}" @click="store.toggleView('编辑')"><i class="fa fa-code"></i> 编辑</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('导图')}" @click="store.toggleView('导图')"><i class="fa fa-map-o"></i> 导图</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('演示')}" @click="store.toggleView('演示')"><i class="fa fa-television"></i> 演示</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('文件')}" @click="store.toggleView('文件')"><i class="fa fa-folder"></i> 文件</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('看板')}" @click="store.toggleView('看板')"><i class="fa fa-list-ul"></i> 看板</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('图谱')}" @click="store.toggleView('图谱')"><i class="fa fa-xing"></i> 图谱</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('甘特')}" @click="store.toggleView('甘特')"><i class="fa fa-list-alt"></i> 甘特</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('月历')}" @click="store.toggleView('月历')"><i class="fa fa-calendar-o"></i> 月历</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('年历')}" @click="store.toggleView('年历')"><i class="fa fa-calendar-o"></i> 年历</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('地图')}" @click="store.toggleView('地图')"><i class="fa fa-location-arrow"></i> 地图</div>
      <div class="button" style="width:calc(100% - 27px)" :class="{active:store.isView('表格')}" @click="store.toggleView('表格')"><i class="fa fa-table"></i> 表格</div>
    </div>
    <div style="display: flex;flex-direction: column;align-items: start;border-bottom: 1px solid var(--borderColor);">
      ...
      <div style="display: flex;width:100%">
        <label>智谱密匙</label>
        <input style="width:calc(100% - 90px)" v-model="store.AIconfig.zhipu_APIkeys" type="password"/>
      </div>
      <div style="display: flex;width:100%">
        <label>声音类型</label>
      ...
      <div style="display: flex;width:100%" v-if="store.AIconfig.tts_type=='GPT_Sovite'">
        <label>{{store.locales=='zh'?'发声音色':'TTS voice:'}}</label>
