  ...
  <div class="settings-container">
    ...
    <div class="settings-content scoll">
      ...
      <div v-if="activeNav === 'general'" class="settings-section">
        ...
        <div class="settings-group">
          ...
          <div class="form-group">
            <label>{{ store.locales=='zh'?'知识库路径' : 'Knowledge base Path' }}</label>
        ...
        <!-- 系统功能 -->
        <div class="settings-group">
          <h3>{{ store.locales=='zh'?'系统功能' : 'System Functions' }}</h3>
          <div class="form-group">
      ...
      <div v-if="activeNav === 'llm'" class="settings-section">
        ...
        <div class="settings-group">
          ...
          <div v-if="store.AIconfig.llm.type === 'ollama'">
            ...
            <div class="form-group">
              ...
              <div class="input-with-button">
                <select v-model="store.AIconfig.llm.ollama.model">
                ...
                <div class="button" style="width:15px;height:18px;margin-top:4px" @click="store.getAIconfig()" 
          ...
          <div class="form-group">
            ...
            <input type="number" v-model="store.AIconfig.llm.max_tokens" min="100" max="8000" 
                   :placeholder="store.locales=='zh'?'例如: 2000' : 'Example: 2000'"/>
          </div>
        ...
        <!-- 功能配置 -->
        <div class="settings-group">
          <h3>{{ store.locales=='zh'?'AI功能配置' : 'AI Function Configuration' }}</h3>
          
          <!-- 功能列表管理 -->
          <div class="form-group">
            <label>{{ store.locales=='zh'?'功能管理' : 'Function Management' }}</label>
            <div class="function-management">
              <!-- 功能列表 -->
              <div class="function-list-container scoll">
                <div 
                  v-for="(func, index) in store.AIconfig.functions" 
                  :key="index"
                  class="function-item"
                ...
                >
                  ...
                  <div class="function-actions">
                    <i class="fa fa-edit" @click.stop="editFunction(index)" :title="store.locales=='zh'?'编辑' : 'Edit'"></i>
                    <i class="fa fa-trash" @click.stop="deleteFunction(index)" :title="store.locales=='zh'?'删除' : 'Delete'"></i>
                  </div>
                </div>
              </div>
              
              <!-- 添加新功能按钮 -->
              <div class="button" @click="addNewFunction" :title="store.locales=='zh'?'添加新功能' : 'Add New Function'">
                <i class="fa fa-plus"></i> {{ store.locales=='zh'?'添加功能' : 'Add Function' }}
              </div>
            </div>
          </div>
          
          <!-- 功能编辑区域 -->
          <div v-if="activeFunctionIndex !== null" class="form-group">
            <label>{{ store.locales=='zh'?'功能编辑' : 'Function Editor' }}</label>
            <div style="width:100%;padding:10px;border:1px solid var(--borderColor);border-radius: 5px;">
              <!-- 功能标题 -->
              <div class="editor-field">
                <label>{{ store.locales=='zh'?'功能标题' : 'Function Title' }}</label>
                <input 
                  type="text" style="padding:8px;margin: 0px;"
                  v-model="store.AIconfig.functions[activeFunctionIndex].title" 
                  :placeholder="store.locales=='zh'?'请输入功能标题' : 'Enter function title'"
                />
              </div>
              
              <!-- prompt设置 -->
              <div class="editor-field">
                <label>{{ store.locales=='zh'?'提示词设定' : 'prompt Setting' }}</label>
                <textarea style="padding:8px;margin: 0px;width:calc(100% - 18px)"
                  v-model="store.AIconfig.functions[activeFunctionIndex].prompt" 
                  rows="4"
                  :placeholder="store.locales=='zh'?'请输入提示词' : 'Enter prompt'"
                ></textarea>
              </div>
              
              <!-- 保存和取消按钮 -->
              <div style="display: flex;gap: 5px;">
                <div class="button" @click="saveFunction" :title="store.locales=='zh'?'保存功能' : 'Save Function'">
                  <i class="fa fa-save"></i> {{ store.locales=='zh'?'保存' : 'Save' }}
                </div>
                <div class="button" @click="cancelEdit" :title="store.locales=='zh'?'取消编辑' : 'Cancel Edit'">
                  <i class="fa fa-times"></i> {{ store.locales=='zh'?'取消' : 'Cancel' }}
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- 高级设置 -->
      <div v-if="activeNav === 'advanced'" class="settings-section">
        <!-- Python环境设置 -->
        <div class="settings-group">
...
const navItems = ref([
  ...
  { id: 'llm', icon: 'fa fa-graduation-cap', title_zh: '模型设置', title_en: 'LLM' },
  { id: 'tts', icon: 'fa fa-volume-up', title_zh: '语音设置', title_en: 'TTS' },
  { id: 'help', icon: 'fa fa-question-circle', title_zh: '软件帮助', title_en: 'Help' },
  { id: 'advanced', icon: 'fa fa-server', title_zh: '高级设置', title_en: 'advanced' }
])
...
// 当前选中的功能索引
const activeFunctionIndex = ref<number | null>(null)
...
const deleteFunction = (index: number) => {
  if (store.locales === 'zh') {
    if (!confirm(`确定要删除功能 "${store.AIconfig.functions[index].title}" 吗？`)) {
      return
    }
  } else {
    if (!confirm(`Are you sure you want to delete function "${store.AIconfig.functions[index].title}"?`)) {
      return
    }
  }
  
  store.AIconfig.functions.splice(index, 1)
  
  if (activeFunctionIndex.value === index) {
...
const addNewFunction = () => {
  const newFunction = {
    title: store.locales === 'zh' ? '新功能' : 'New Function',
    prompt: '',
  }
  
  store.AIconfig.functions.push(newFunction)
  activeFunctionIndex.value = store.AIconfig.functions.length - 1
}

// 保存功能配置
const saveFunction = () => {
  if (activeFunctionIndex.value !== null) {
    const func = store.AIconfig.functions[activeFunctionIndex.value]
    
    if (!func.title.trim()) {
      if (store.locales === 'zh') {
        console.log('功能标题不能为空！')
      } else {
        console.log('Function title cannot be empty!')
      }
      return
    }
    ...
    if (store.locales === 'zh') {
      ...
      console.log('Function configuration saved!')
    }
  }
}
...
// 监听模型类型变化，重置连接状态
watch(() => store.AIconfig.llm.type, (newType) => {
  store.AIconfig.llm.online = false
  switch(newType) {
    case 'ollama':
      store.AIconfig.llm.ollama.model = ''
...
print("Test successful!")`
  ...
  try {
    const result = await window.ipcRenderer.invoke('executePythonCodeWithEnvironment', {
      code: testCode,
      environment: store.TrustedPython
    })
